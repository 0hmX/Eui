...
3.  **Set up Python Environment & Install Dependencies:**
    ...
    uv venv
    # Activate the virtual environment (uv might show you the command, or use standard activation)
    # On Windows:
    ...
    uv sync
    ```
    If an environment setup script like [env.sh](env.sh) exists and is relevant to your OS, source it after activating the virtual environment:
    ```bash
    # Example for bash-like shells
    # source env.sh
    ```
...
```bash
uv run python src/tools/audio_tool.py
```
...
**How to Run:**
```bash
uv run python src/tools/render_manim_tool.py
```
...
4.  **Adhere to project standards.** This project uses Ruff for linting and formatting, managed via `uv`.
    ```bash
    # Format code
    uv run ruff format .
...
7.  **Open a Pull Request (PR)** against the main Eui repository, detailing your changes, why they are needed, and how you tested them.
...
```<!-- filepath: c:\Users\ankan\Documents\Projects\Eui\README.md -->
# Eui: The Automated Content Framework
